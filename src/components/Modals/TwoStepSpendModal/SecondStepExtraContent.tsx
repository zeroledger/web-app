...
export const SecondStepExtraContent = ({
  ...
  minimalTransactionDetails,
}: {
  isDetailsOpen: boolean;
  setIsDetailsOpen: (isOpen: boolean) => void;
  fullTransactionDetails: { label: string; value: string | ReactNode }[];
  minimalTransactionDetails: { label: string; value: string | ReactNode }[];
}) => {
  return (
    <div className="bg-gray-700 rounded-lg border border-gray-600 mb-6 overflow-hidden">
      <div className="p-4">
        <div className="space-y-3 text-sm">
          {minimalTransactionDetails.map((detail) => (
      ...
      >
        <div className="flex justify-between items-center">
          <h3 className="text-white font-semibold">More Details</h3>
      ...
      >
        <div className="p-4">
          <div className="space-y-3 text-sm">
            {fullTransactionDetails.map((detail) => (
              <div key={detail.label} className="flex justify-between">
                <span className="text-gray-400">{detail.label}:</span>
                <span className="text-white font-mono break-all text-right ml-2">
                  {detail.value}
